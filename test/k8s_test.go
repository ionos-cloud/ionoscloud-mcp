...
func TestK8sToolEndpoints(t *testing.T) {
	...
	tests := []toolTest{
		{name: "list_k8s_clusters", args: map[string]any{}, wantMethods: []string{"GET"}, wantPaths: []string{"/cloudapi/v6/k8s"}},
		...
		{name: "list_k8s_nodepools", args: map[string]any{"k8s_cluster_id": cluster}, wantMethods: []string{"GET"}, wantPaths: []string{"/cloudapi/v6/k8s/" + cluster + "/nodepools"}},
		...
		{name: "list_k8s_nodepool_nodes", args: map[string]any{"k8s_cluster_id": cluster, "nodepool_id": np}, wantMethods: []string{"GET"}, wantPaths: []string{"/cloudapi/v6/k8s/" + cluster + "/nodepools/" + np + "/nodes"}},
