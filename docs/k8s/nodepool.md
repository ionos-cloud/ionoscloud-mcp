...
|---|---|
| Maintenance window, labels, annotations, taints | quick, but still asynchronous |
...
| `confirmation_token` | string | No | Omit on the first call; pass the token returned by the preview on the second. Bound to every field in the preview, so nothing may change between the two calls |
...
>
> Node **taints** work the same way: supply `taints` to replace them, or omit the field to keep whatever the pool carries — including taints applied out of band, which a replacing PUT would otherwise drop.
>
...
> **`NoExecute` evicts pods that are already running.** Of the three taint effects only `NoSchedule` (block new pods without a matching toleration) and `PreferNoSchedule` (avoid them where possible) are limited to scheduling decisions — `NoExecute` also removes pods already on the pool's nodes. Adding one to a running pool takes effect as soon as the update is applied, so confirm it with the user first.
...
| `taints` | object[] | No | **Replaces** the node taints. Omit to keep them; pass `[]` to remove them all |
