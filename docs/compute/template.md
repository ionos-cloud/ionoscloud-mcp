...
| `filters` | object | No | Server-side property filters; e.g. `{"name":"cubeXS","cpuFamily":"INTEL_SKYLAKE"}`. If the result is empty, retry without filters — a filter typo or mismatch silently returns nothing. |
