...
func RegisterUsageTools(server *mcp.Server, client *sdk.APIClient) {
	mcp.AddTool(server, &mcp.Tool{
		Name:        "billing_usage",
		Description: "Get aggregated resource usage for your contract for the current billing period. Shows metered quantities (CPU hours, GB-hours, etc.) grouped by datacenter.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get aggregated resource usage for a specific datacenter (VDC UUID) in the current billing period. Use billing_usage first to find datacenter IDs.",
