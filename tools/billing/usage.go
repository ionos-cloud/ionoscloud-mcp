...
func RegisterUsageTools(server *mcp.Server, client *sdk.APIClient) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get aggregated resource usage for a specific datacenter (VDC UUID) in the current billing period. Use list_billing_usage first to find datacenter IDs. For FOCUS v1.3 compliant output, read resource ionos://billing/focus-v1.3.",
