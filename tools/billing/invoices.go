...
func RegisterInvoiceTools(server *mcp.Server, client *sdk.APIClient) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "List all invoices for your IONOS Cloud contract. Returns invoice IDs, dates, and amounts.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "List invoices for a specific billing period (YYYY-MM). One month per call. If the user requests a range longer than one month, calculate the number of monthly calls required, inform the user, and ask for permission before proceeding.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get the detailed line-item breakdown for a specific invoice by ID. Use billing_invoices first to find available invoice IDs.",
