...
func RegisterUtilizationTools(server *mcp.Server, client *sdk.APIClient) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get high-granularity resource utilization for your contract for the current billing period. Shows per-resource metrics (CPU, RAM, storage, DNS) grouped by datacenter.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get high-granularity resource utilization for a specific billing period (YYYY-MM). One month per call. If the user requests a range longer than one month, calculate the number of monthly calls required, inform the user, and ask for permission before proceeding.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get high-granularity resource utilization for a specific date (YYYY-MM-DD). Use this for day-level analysis within a month.",
