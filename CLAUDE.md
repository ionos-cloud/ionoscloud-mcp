...
The presentation half of the two-phase flow lives in `tools/preview.go` (`tools.Preview`, `tools.Fields`, `tools.BlastRadius`, `tools.ConfirmErrorText`, `tools.Target`, `tools.HasToken`, `tools.DeletedAsync`, `tools.Opt*`). It is product-agnostic on purpose — DNS write tools use it unchanged, and cert and object storage should do the same rather than growing their own preview format, so a model only has to learn one shape.
