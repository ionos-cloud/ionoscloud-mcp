...
- **Fields the spec marks `x-internal` are never exposed as tool inputs.** Four remain in the CloudAPI spec — `vnet`, `placementGroupId`, `vni` and `KubernetesNodePoolLan.datacenterId` — each documented "requires system privileges, for internal usage only". The generated SDK models them anyway, so presence in the SDK is not evidence a field is callable; the internal-usage sentence in the field's comment is the tell.

  Node pool `taints` was the worked example until IONOS published it, and it is now the worked example for the reverse. The signals that it graduated: it is **absent** from compute SDK v2.0.5 and appears in v2.0.7 with a customer-facing description and a documented limit (max 50 per pool) rather than the internal-usage sentence, and the Terraform provider exposes it to customers ([#1039](https://github.com/ionos-cloud/terraform-provider-ionoscloud/pull/1039)). `create_k8s_nodepool` and `update_k8s_nodepool` accept it. Note what did **not** change: the update still carries the pool's existing taints forward when the caller omits the field, because accepting a field is not a reason to stop protecting it on a replacing PUT.
...
**Bind a confirmation token to every input that changes what the operation does, not just the identifying ones.** `tools.Target` takes as many parts as needed, and anything left out is a field the execute call is free to change after a human approved the preview — which defeats the point of showing one. A narrow target reads as correct, so only a test that swaps a field between the two calls catches it. A field belongs in the target unless it is genuinely cosmetic (`create_dns_zone` binds `zone_name` but not `description`). `create_cert_auto_certificate` binds `keyAlgorithm` and the subject alternative names, because those decide what the issued certificate covers; `create_cert_provider` binds the external account binding, hashing the secret rather than storing it verbatim — the same reasoning as `import_dns_zone_file` hashing the zone file. `TestCreateCertAutoCertificateTokenIsBoundToWhatIsIssued` and `TestCreateCertProviderTokenIsBoundToTheBinding` assert a swapped field is refused and mutates nothing.

**A tool does not ship if calling it requires putting a secret into the model's context.** This is the second reason a tool can be declined, alongside "no typed SDK call can produce an acceptable body" (`update_ip_block`). `create_cert_certificate` is the worked example: `certificatesPost` requires `privateKey` in the body, so the tool would have to accept a TLS private key as an argument, which lands it in the conversation, the plaintext session transcript on disk, and every subsequent request to the model provider. `tools/cert/redact.go` cannot help — redaction guards the response and the preview, both outbound. The register function documents the omission and `TestCreateCertCertificateIsNotRegistered` asserts it stays absent at every scope. Where the API offers a route that keeps the secret server-side, prefer it and say so: `create_cert_auto_certificate` has IONOS generate and hold the key, and `create_dns_zone_dnssec_key` likewise. Four inbound-secret fields remain, all on compute creates (`image_password` and `user_data` on `create_server`/`create_volume`) and one on `create_cert_provider`; they are lower-value credentials, they render through `tools.Redacted` in previews, and their descriptions should not imply the inbound path is protected.
