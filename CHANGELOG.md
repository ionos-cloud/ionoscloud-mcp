...
- **Kubernetes node pool taints** on `create_k8s_nodepool` and `update_k8s_nodepool`. Each taint takes a `key`, an optional `value` and an `effect` (`NoSchedule`, `NoExecute` or `PreferNoSchedule`), at most 50 per pool; the effect is matched case-insensitively and normalised to the API's spelling, and keys and values are checked against the Kubernetes label grammar before the call goes out. On the update, `taints` behaves like the other list-valued fields — supplying it replaces the pool's taints, an empty array removes them all, and omitting it keeps whatever the pool carries. IONOS previously marked the field internal, so no tool accepted it; it is public as of compute SDK v2.0.7.

- **Write operations for Certificate Manager**, off by default and behind the same `IONOS_MCP_TOOL_SCOPE` gate: rename and delete for certificates, plus create, update and delete for auto-certificates and ACME providers. Every create and delete takes two calls — the first returns a preview and a one-time token, the second carries it. The 6 existing Certificate Manager read tools now carry the `readOnlyHint` annotation.
