...
### Improvements
- `[IMP][ionoscloud-mcp]` Compact billing utilization/usage tools: drop redundant fields, hoist `meter_definitions` to top level, flatten `quantity` object, filter zero-quantity meters by default (use `include_zero=true` to keep them). Add `group_by` (`meter`/`datacenter`), `datacenter_id`, `meter_types`, `regions` flags for finer scoping. Affects `list_billing_utilization`, `list_billing_utilization_by_period`, `get_billing_utilization_daily`, `list_billing_usage`, `get_billing_usage_by_datacenter`. Typical response shrinks from ~1.27 MB to ~325 KB (default) or ~10 KB (group_by=datacenter) - @avirtopeanu-ionos
