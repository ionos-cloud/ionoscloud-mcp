...
- **`update_ip_block` removed: it could never have worked.** Renaming an IP block failed on every call against the real API, and the mock-based tests passed because they asserted the wrong thing. `IpBlockProperties` models `location` and `size` as non-pointer fields and `ToMap` serializes both unconditionally, so the smallest body the typed model can produce is `{"name":…,"location":…,"size":…}` — and the API documents `location` as *"disallowed in update requests"*, wording that sits in the SDK's own comment on the field. It rejects the request for carrying the immutable fields at all, whatever their values. `IpblocksPut` is no escape: `IpBlock` embeds `Properties` as a non-pointer field and serializes it unconditionally too.

  A previous change to this tool read the same field comment as "send the current values so the API sees no change" and added a read-then-carry-forward, with a test asserting `{"name","location","size"}` was the correct body. The comment's parenthetical says *disallowed in update requests*, which is the opposite instruction, so that test locked in the broken behaviour — and no mock-based test could have caught it, because the mock does not enforce the constraint. Replaced with `TestUpdateIpBlockIsNotRegistered`.

  The older `sdk-go/v6` line models both fields as pointers and guards them when marshalling, which is how the Terraform provider renames a block with a body containing only the name. Compute bundle `v2.0.5` and `v2.0.7` both have the defect, so there is no version to bump to. Reserving a block with the name you want (`create_ip_block` takes `name`) is the workaround; renaming afterwards needs the Terraform provider, ionosctl or the DCD. Reported upstream.
...
- **Previews no longer claim a create will destroy things.** `create_security_group_rule` told the caller that confirming it would destroy the NIC assigned to the group — the exact opposite of what the call does. The counts were right; the framing was invented by the renderer, which wrapped *every* blast-radius list in "Contained resources that will be destroyed". That was correct for a data center delete and false everywhere the entries survive: a LAN delete listed "NICs that will lose their network connection" under a destruction heading, as did the IP-block, cross-connect and load-balancer deletes.
  ...
  This matters beyond wording. A caller that meets a false warning, investigates and correctly concludes it was bogus has done the right thing on the evidence — and has learned to discount the section, which is what makes the next warning ineffective. A preview is only load-bearing while it is accurate, so `TestAffectedRadiusNeverClaimsDestruction` and `TestBareRadiusFailsSafe` pin both directions.
